...
rf_mod_randomforest<-randomForest(Classes ~ .,data = TreeSpecLib_derivs, ntree=1000,importance=TRUE) # OOB prediction error 10.42 % 
 ...
 RandomForest_reduced_confusionmatrix<-rf_mod_randomforest_reduced$confusion%>%as.data.frame()
 ...
 Ranger_reduced_confusionmatrix<-rf_mod_ranger_reduced$confusion.matrix%>%as.data.frame.matrix()
