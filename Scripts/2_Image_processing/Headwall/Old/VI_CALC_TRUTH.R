...
MSGC_PLOT1  <-read.csv("Outputs/2_Imagery/Headwall/Processing/MSGC_PLOT1_GtRUTH.csv")
...
MSGC_PLOT1_matrix   <-as.matrix(MSGC_PLOT1   [-1:-3])
...
MSGC_PLOT1_speclib   <-speclib  (MSGC_PLOT1_matrix   ,HDW_ng_wv[1:272])
...
MSGC_PLOT1_VIs       <-vegindex(MSGC_PLOT1_speclib       ,index=VIs)

##rename columns
colnames(MSGC_PLOT1_VIs  )<-VIs

##lets do a logical test on MSGC_PLOT1_VIs to see if strange values exist
##Here we need an ifelse function to remove NaNs and Inf or possibly convert them to NAs/0s
test3<-lapply(MSGC_PLOT1_VIs,range)%>%as.data.frame%>%t()%>%as.data.frame
##test3%>%View()##There are no columns where NaNs and Inf exist in this dataframe

#lets REplace NANS AND INFS with 0s
MSGC_PLOT1_VIs[sapply(MSGC_PLOT1_VIs, is.infinite)]  <- 0
MSGC_PLOT1_VIs[sapply(MSGC_PLOT1_VIs, is.na)]        <- 0

##we need to combine lat/long columns with our new VI variables
MSGC_PLOT1_VIs  <-cbind(MSGC_PLOT1   [1:3],MSGC_PLOT1_VIs   )
...
colnames(MSGC_PLOT1_VIs )[-1:-3]<-newcolnames  ##dim 66024 97

##Now that we have our VIs calculated we can go ahead and export these dataframes
write.csv(MSGC_PLOT1_VIs             ,"Outputs/2_Imagery/Headwall/Processing/MSGC_PLOT1_VIs.csv",row.names = FALSE)
