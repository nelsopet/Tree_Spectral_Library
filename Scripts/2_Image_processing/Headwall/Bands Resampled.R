...
shadows_1<-brick("Original_data/Headwall/shadow_1")%>%rasterToPoints()%>%as.data.frame()%>%mutate(species="shadow")%>%dplyr::select(x,y,species,everything())
shadows_2<-brick("Original_data/Headwall/shadow_2")%>%rasterToPoints()%>%as.data.frame()%>%mutate(species="shadow")%>%dplyr::select(x,y,species,everything())
...
MSGC_PLOT1_005nm[,-c(1,2,3)][MSGC_PLOT1_005nm[, -c(1,2,3)] < 0] <- 0
MSGC_PLOT1_010nm[,-c(1,2,3)][MSGC_PLOT1_010nm[, -c(1,2,3)] < 0] <- 0
MSGC_PLOT1_050nm[,-c(1,2,3)][MSGC_PLOT1_050nm[, -c(1,2,3)] < 0] <- 0
MSGC_PLOT1_100nm[,-c(1,2,3)][MSGC_PLOT1_100nm[, -c(1,2,3)] < 0] <- 0

###Lets save our new dfs
write.csv(MSGC_PLOT1,       "Outputs/2_Imagery/Headwall/Processing/MSGC_PLOT1_GtRUTH.csv", row.names = FALSE)
