...
#'
#' @inheritParams None
#' @return A dataframe with the spectral components
...
resample_spectral_dataframe <- function(
  ...
  colnames(resampled_df_no_metadata) <- paste0(
    "X",
...
#' Extracts indices from dataframe column names
#' 
#' Long description here
#' 
#' @inheritParams None
...
#' Converts dataframe to a spectral library
#' 
#' @inheritDotParams 
...
#' Converts a spectral library to a dataframe. This function is not yet implemented
#' 
#' @inheritDotParams 
#' @return A dataframe
...
#' @param headwall: The headwall indices (bands) of interest.
...
.calculate_veg_index_from_df <- function(df,
  ...
  calculatedVegIndex <- foreach() #Something isn't right, missing arguments
}
...
#' Removes columns outside the min and max indices
#'
#' Long description here
#'
#' @return 
...
#'
#' @return 
#' @param df: A dataframe. Assumes that the target data is in column 'z'
#' @seealso None
#' @export 
#' @examples Not yet implemented
...
#' adjoin 16 random characters to the file name to avoid collision.
#'
#' @return 
#' @param df: A dataframe
...
#' If FALSE, the data.frame is saved to disk, but no conversion is made.
#' @seealso None
#' @export 
#' @examples Not yet implemented

convert_and_save_output <- function(df, aggregation_level, 
                                    save_path = NULL, 
                                    return_raster = TRUE, 
                                    target_crs = NULL){
  ...
  # convert the prediction values to integers
...
#'
#' @return 
#' @param prediction_df: A dataframe of prediction from the model.  It should have a single column of data.
#' @param base_df: The base dataframe for creating the prediction; 
#' it should have columns x and y specifying the spatial coordinates for that row.
#' @return a dataframe with three columns: 'x', 'y', and 'z'.  'x' and 'y' are the spatial location of the 
...
#'
#' @return 
#' @param aggregation: An integer that specifies the aggregation level for the output raster.
#' Should be 1 (functional group 1), 2 (functional group 2), 3 (genus), or 4 (species).
#' @seealso None
#' @export 
#' @examples Not yet implemented
...
drop_zero_rows <- function(df) {
  ...
  # sort the columns to make sure x and y are first before calculating row sums
  df_rowsums <- df %>% dplyr::relocate(y) %>% dplyr::relocate(x)
  
  num_cols = ncol(df)
  
  # return only the rows with row sums over zero
